...
class TurboM(Turbo1):
    ...
    def optimize(self, max_time):
        ...
        while it < self.n_trust_regions and time.monotonic() - startT < max_time:
            ...
            wallclocks.extend([time.monotonic() - startT]*self.n_init)
            self._idx = np.vstack((self._idx, it * np.ones((self.n_init, 1), dtype=int)))
            self.n_evals += self.n_init

            if self.verbose:
                fbest = fX_init.min()
                print(f"TR-{it} starting from: {fbest:.4}")
                sys.stdout.flush()
            it += 1
        # Thompson sample to get next suggestions
        while self.n_evals < self.max_evals and time.monotonic() - startT < max_time:
            ...
            wallclocks.extend([time.monotonic() - startT]*self.batch_size)
