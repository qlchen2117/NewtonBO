...
from .environment_api import EnvironmentObjective
from .acquisition_function import GradientInformation
from .model import ExactGPSEModel, DerivativeExactGPSEModel


def fit_gpytorch_by_adam(mll, model, NUM_EPOCHS=50):
...
class BayesianGradientAscent(AbstractOptimizer):
    ...
    def __init__(
        ...
        self.optimizer4mll = optimizer4mll
        ...
        self.train_xs, self.train_ys = torch.empty(0, self.D).to(self.params), torch.empty(0).to(self.params)
        # Initialization of training data.
        self.startT = time.monotonic()
        self.wallclocks = []
        if generate_initial_data is not None:
            train_x_init, train_y_init = generate_initial_data(self.objective)
            self.model.append_train_data(train_x_init, train_y_init)
            self.store_samples(train_x_init, train_y_init)
    ...
    def step(self) -> None:
        ...
        self.params = torch.clamp(self.params, 0., 1.)
        f_params = torch.tensor([self.objective(xx) for xx in self.params]).to(self.params)

        if self.verbose:
            print(f"Reward of parameters theta_(t-1): {f_params.item():.2f}.")
        self.model.append_train_data(self.params, f_params)
        self.store_samples(self.params, f_params)
        ...
        if self.optimize_hyperparamters and (
            ...
            if self.optimizer4mll == 'scipy':
                try:
                    message = fit_gpytorch_mll(mll)
                    if message.training:  # if fail to fit
                        fit_gpytorch_by_adam(mll, self.model)
                except ModelFittingError:
                    fit_gpytorch_by_adam(mll, self.model)
            elif self.optimizer4mll == 'Adam':
                fit_gpytorch_by_adam(mll, self.model)
            else:
        ...
        for i in range(self.max_samples_per_iteration):
            ...
            new_y = torch.tensor([self.objective(x) for x in new_x]).to(new_x)

            # Update training points.
            self.model.append_train_data(new_x, new_y)
            self.store_samples(new_x, new_y)
    ...
    def store_samples(self, train_x, train_y):
        self.train_xs = torch.cat((self.train_xs, self.model.unnormalize(train_x)))
        self.train_ys = torch.cat((self.train_ys, train_y))
        self.wallclocks.extend([time.monotonic()-self.startT]*train_x.shape[0])