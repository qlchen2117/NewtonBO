    ...
    for i in range(n):
        ...
        with gpytorch.settings.max_cholesky_size(2000):
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", category=UserWarning)  # UserWarning: The .grad attribute of a Tensor that is not a leaf Tensor is being accessed.
                with torch.no_grad():
...
class ALEBO:
    ...
    def gen(
        ...
        objective_weights = torch.tensor([-1.]).to(self.B)
        ...
        model = self.model
        acquisition_function = ei_or_nei(
            model=model,
            objective_weights=objective_weights,
        ...
        candidates, expected_acquisition_value = alebo_acqf_optimizer(
