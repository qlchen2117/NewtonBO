...
class KSIRBO:
    ...
    def update_model(self, f_t, final_x_at_minY, batch_size):
        ...
        self.emb[:self.num+batch_size] = final_x_at_min
        self.fvals = np.vstack((self.fvals, f_t))
        self.trainY = (self.fvals - self.fvals.mean()) / self.fvals.std(ddof=1)
        self.num += batch_size

        idx = f_t.argmin()
        f_t_min, x_min = f_t[idx], final_x_at_minY[idx]
        ...
        self.update_kernel(final_x_at_min[-batch_size:], learn_hyper)
...
def opt(objective: Callable, num_iter: int, max_time, batch_size: int, model, startT, wallclocks):
    ...
    while it < num_iter and time.monotonic() - startT < max_time:
        candidates, _ = maximize_kacq(model, dopt, 'ei')
...
def ksir_bo(obj_fct, dim, high_dim, init_size, n_iter, max_time, batch_size):
