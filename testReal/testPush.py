...
""" Run Robot push. Maximization problems. """
# import warnings
...
import numpy as np
from benchmarks.push_function import PushReward


# # Save Results
from pathlib import Path
RES_PATH = Path("results/push/")
if not RES_PATH.exists():
    RES_PATH.mkdir(parents=True)


def testPush(N_EPOCH: int, N_ITERACTIONS: int, N_INIT: int, BATCH_SIZE: int, methods):
    func_name = 'push'
    problem = PushReward()
    lb, ub = np.array(problem.xmin), np.array(problem.xmax)
    DIM = len(lb)
    EM_DIM = 10
    ...
    def eval_objective(x: Tensor):
        ...
        return problem(x_np, False)

    def eval_objective4min(x: Tensor):
        x_np = lb + (ub - lb) * x.numpy()
        return problem(x_np, False) * -1  # Flip the value for minimization
    ...
    def eval_objective4alebo(parameterization):  # wrapper for alebo
        ...
        y = problem(lb + (ub - lb) * x_np, False)
        return {"objective": (y, 0.0)}

    def eval_objective4min_np(x: np.ndarray):  # wrapper for hebo
        """x is assumed to be in [0, 1]^d"""
        return problem(lb + (ub - lb) * x, False) * -1 # Flip the value for minimization
    ...
    if 'gibo' in methods:
        ...
        for i in range(N_EPOCH):
            _, Y = gibo(objective=eval_objective, dim=DIM, n_iterations=N_ITERACTIONS, n_init=N_INIT, batch_size=BATCH_SIZE, optimizer4mll='Adam')
            Y_np = -1 * Y.cpu().numpy()
    ...
    if 'gp-ei' in methods:
        ...
        for i in range(N_EPOCH):
            ...
            Y_np = Y.cpu().numpy() * -1
            store_data[i] = Y_np.ravel()
        np.save(RES_PATH / func_name+f"-D{DIM}-gp-ei.npy", store_data)
