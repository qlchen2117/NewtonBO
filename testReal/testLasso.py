...
def testLasso(pick_data: str, N_EPOCH: int, N_ITERACTIONS: int, N_INIT: int, BATCH_SIZE: int, methods):
    ...
    if 'saasbo' in methods:
        ...
        np.save(res_p / f"{pick_data}-D{DIM}-saasbo.npy", store_data)
    ...
    if 'rembo' in methods:
        ...
        np.save(res_p / f"{pick_data}-D{DIM}-d{EM_DIM}-rembo.npy", store_data)
    ...
    if 'alebo' in methods:
        ...
        for i in range(N_EPOCH):
            ...
            store_data[i] = Y_np.ravel()
            # wallclocks[i] = clock.cpu().numpy()
        np.save(res_p / f"{pick_data}-D{DIM}-d{EM_DIM}-alebo.npy", store_data)
    ...
    #     Y_np = -1 * Y.cpu().numpy()
    #     store_data[i] = Y_np.ravel()
    # np.save(res_p / f"{pick_data}-D{DIM}-d{EM_DIM}-kdr_bo.npy"), store_data)
    ...
    #     Y_np = -1 * Y.cpu().numpy()
    #     store_data[i] = Y_np.ravel()
    # np.save(res_p / f"{pick_data}-D{DIM}-d{EM_DIM}-mkdr_bo.npy"), store_data)
    ...
    #     Y_np = -1 * Y.cpu().numpy()
    #     store_data[i] = Y_np.ravel()
    # np.save(res_p / f"{pick_data}-D{DIM}-d{EM_DIM}-mkdr_bo_scale.npy"), store_data)
    ...
    if 'turbo1' in methods:
        ...
        np.save(res_p / f"{pick_data}-D{DIM}-turbo1.npy", store_data)
    ...
    if 'turbom' in methods:
        ...
        np.save(res_p / f"{pick_data}-D{DIM}-turbom.npy", store_data)
    ...
    if 'turbo-d' in methods:
        ...
        for i in range(N_EPOCH):
            ...
            np.save(res_p / f"{pick_data}-D{DIM}-turboD{i}.npy", Y.view(-1).cpu().numpy())
    ...
    if 'turbo-d-drop' in methods:
        ...
        np.save(res_p / f"{pick_data}-D{DIM}-turboD-drop.npy", store_data)
    ...
    if 'gibo' in methods:
        ...
        np.save(res_p / f"{pick_data}-D{DIM}-gibo.npy", store_data)
    ...
    if 'sobol' in methods:
        ...
        for i in range(N_EPOCH):
            ...
            store_data[i] = Y_np.ravel()
            # wallclocks[i] = clock.cpu().numpy()
        np.save(res_p / f"{pick_data}-D{DIM}-sobol.npy", store_data)
    ...
    if 'sir-bo' in methods:
        ...
        np.save(res_p / f"{pick_data}-D{DIM}-d{EM_DIM}-sir_bo.npy", store_data)
    ...
    if 'ksir-bo' in methods:
        ...
        for i in range(N_EPOCH):
            ...
            store_data[i] = Y.ravel()
        np.save(res_p / f"{pick_data}-D{DIM}-d{EM_DIM}-ksir_bo.npy", store_data)
    ...
    if 'add-gp-ucb' in methods:
        ...
        for i in range(N_EPOCH):
            ...
            # wallclocks[i] = clock
        np.save(res_p / f"{pick_data}-D{DIM}-add_bo.npy", store_data)

    if 'gp-ei' in methods:
        from BO.bo import bo
        for i in range(N_EPOCH):
            _, Y, _ = bo(eval_objective, ndims=DIM, n_init=N_INIT, n_iterations=N_ITERACTIONS, max_time=np.inf, batch_size=BATCH_SIZE, acq='EI')
            Y_np = Y.cpu().numpy() * -1
            np.save(res_p / f"{pick_data}-D{DIM}-gp-ei{i}.npy", Y_np.ravel())
    ...
    if 'cmaes' in methods:
        ...
        for i in range(N_EPOCH):
            ...
            np.save(res_p / f"{pick_data}-D{DIM}-cmaes{i}.npy", Y.ravel())
            # np.save(res_p / f"{type(func).__name__}-full-D{DIM}-cmaes-clock{i}.npy"), clock)

## HDBO
# np.save(res_p / f"lasso-dna-D{DIM}-d{EM_DIM}-turbo.npy"), Y_turbo)
# np.save(res_p / f"lasso-dna-D{DIM}-d{EM_DIM}-mkdr_bo.npy"), Y_mkdr_bo)
