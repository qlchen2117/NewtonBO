"""
Run RL benchmarks (mujoco). Maximization problems.
"""
# import warnings
...
from benchmarks.rl_benchmark import RLBenchmark, RLEnv

# # Save Results
from pathlib import Path
RES_PATH = Path("results/mujoco/")
if not RES_PATH.exists():
    RES_PATH.mkdir(parents=True)


def testRL(func_name: str, N_EPOCH, N_ITERACTIONS: int, N_INIT: int, BATCH_SIZE: int, methods):
    ...
    lb, ub = rlProblem.lb, rlProblem.ub

    def eval_objective(x: Tensor):
        x_np = lb + (ub - lb) * x.numpy()
        return rlProblem(x_np)

    def eval_objective4min(x: Tensor):
        x_np = lb + (ub - lb) * x.numpy()
        return rlProblem(x_np) * -1  # Flip the value for minimization
    ...
    def eval_objective4alebo(parameterization):  # wrapper for alebo
        ...
        y = rlProblem(lb + (ub - lb) * x_np)
        return {"objective": (y, 0.0)}

    def eval_objective4min_np(x: np.ndarray):  # wrapper for hebo
        """x is assumed to be in [0, 1]^d"""
        return rlProblem(lb + (ub - lb) * x) * -1 # Flip the value for minimization
    ...
    if 'alebo' in methods:
        ...
        for i in range(N_EPOCH):
            ...
            store_data[i] = Y_np.ravel()
            # wallclocks[i] = clock.cpu().numpy()
        np.save(RES_PATH / func_name+f"-D{DIM}-d{EM_DIM}-alebo.npy", store_data)
    ...
    if 'gibo' in methods:
        ...
        np.save(RES_PATH / func_name+f"-D{DIM}-gibo.npy", store_data)
    ...
    if 'gp-pes' in methods:
        from BO.bo import bo
        for i in range(N_EPOCH):
            _, Y = bo(eval_objective, ndims=DIM, n_init=N_INIT, n_iterations=N_ITERACTIONS, batch_size=BATCH_SIZE, acq='PES', optimizer='CMA-ES')
            Y_np = Y.cpu().numpy() * -1
            store_data[i] = Y_np.ravel()
        np.save(RES_PATH / func_name+f"-D{DIM}-gp-pes.npy", store_data)
