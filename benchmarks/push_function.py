...
def main():
    ...
    f = PushReward()
    xs = np.random.uniform( f.xmin, f.xmax, size=(100, len(f.xmin)) )
    # print('Input = {}'.format(x))
    startT = time.monotonic()
    ys = [f(xx, False) for xx in xs]
    print(f"use time: {time.monotonic()-startT}")
    print(f'Output = {ys}. Max: {max(ys)}. Min: {min(ys)}')
    print(f"Maximum of the objective: {f.f_max}")
